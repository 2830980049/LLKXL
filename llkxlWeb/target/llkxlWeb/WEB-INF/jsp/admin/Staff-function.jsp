                    ...
                    <table id="my_order" class="table table-bordered table-striped">
                        ...
                        <c:forEach items="${list}" var="staff">
                            <tr>
                                ...
                                <td>${staff.mbPhone}</td>
                                <td>${staff.cpMail}</td>
                                <td>${staff.otherMail}</td>
                                <td>${staff.qqNumber}</td>
                                <td><a href="#" onclick="sub(${staff.loginName});" style="color: #606c84;font-weight: bold">编辑</a> <a href="#" onclick="subs(${staff.loginName});" style="color: crimson;font-weight: bold">删除</a></td>
                            </tr>
                        </c:forEach>
                        </tbody>
                    </table>
...
<script>
    function sub(st) {
        var loginName = st;
        window.location.href = "<%=ctxPath%>/admin/updatas?loginName="+loginName;
    }
    function subs(st) {
        var loginName = st;
        var result = window.confirm("您确定删除该员工信息?");
        if (result) {
            window.location.href = "<%=ctxPath%>/admin/deletes?loginName="+loginName;
        }
    }
</script>
</body>
